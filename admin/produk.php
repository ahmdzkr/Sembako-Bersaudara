...
$edit = $produk !== null;
$v    = $produk ?? [
    'nama' => '', 'kategori' => '', 'harga' => '', 'satuan' => '', 'harga_satuan' => '',
    'isi_kg' => '', 'stok' => '', 'deskripsi' => '', 'gambar' => null,
];
$err  = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!$_POST && (int) ($_SERVER['CONTENT_LENGTH'] ?? 0) > 0) {
        ...
        $v['isi_kg']       = trim($_POST['isi_kg'] ?? '');
        $v['stok']         = trim($_POST['stok'] ?? '');

        $kat = $v['kategori'];
        $pakaiKg     = in_array($kat, ['Fresh Good', 'Dry Good'], true);
        $pakaiSatuan = in_array($kat, ['Dry Good', 'Lainnya'], true);
        $pakaiIsiKg  = $kat === 'Dry Good';

        if ($v['nama'] === '' || mb_strlen_aman($v['nama']) > 150) { $err['nama'] = 'Isi nama produk (maksimal 150 huruf).'; }
        if (!in_array($kat, KATEGORI, true)) { $err['kategori'] = 'Pilih salah satu kategori.'; }

        $harga = $hargaSatuan = $isiKg = null;

        if ($pakaiKg) {
            $harga = angka_atau_null($v['harga']);
            if ($harga === null) { $err['harga'] = 'Harga per kg harus angka bulat, 0 atau lebih.'; }
        }
        if ($pakaiSatuan) {
            if ($v['satuan'] === '' || mb_strlen_aman($v['satuan']) > 20) { $err['satuan'] = 'Isi nama satuan, misalnya karung, pak, atau dus (maksimal 20 huruf).'; }
        ...
        if ($pakaiIsiKg) {
            $isiKg = angka_atau_null($v['isi_kg']);
            if ($isiKg === null || $isiKg < 1) { $err['isi_kg'] = 'Isi berat 1 satuan dalam kg, minimal 1.'; }
        }

        $stok = filter_var($v['stok'], FILTER_VALIDATE_INT, ['options' => ['min_range' => 0, 'max_range' => 1000000]]);
        if ($stok === false) {
            $err['stok'] = $kat === 'Lainnya'
                ? 'Stok harus angka bulat (jumlah satuan), 0 atau lebih.'
                : 'Stok harus angka bulat (kg), 0 atau lebih.';
        }
        ...
        if (!$err) {
            $satuanSimpan = $pakaiSatuan ? $v['satuan'] : null;
            if ($edit) {
                $q = db()->prepare('UPDATE products SET nama=?, kategori=?, harga=?, satuan=?, harga_satuan=?, isi_kg=?, stok=?, deskripsi=?, gambar=? WHERE id=?');
                $q->execute([$v['nama'], $kat, $harga, $satuanSimpan, $hargaSatuan, $isiKg, $stok, $v['deskripsi'] ?: null, $gambar, $id]);
                if ($gambar !== $gambarLama) { hapus_file_foto($gambarLama); }
                $_SESSION['flash'] = ['success', 'Perubahan pada “' . $v['nama'] . '” disimpan.'];
            } else {
                $q = db()->prepare('INSERT INTO products (nama, kategori, harga, satuan, harga_satuan, isi_kg, stok, deskripsi, gambar) VALUES (?,?,?,?,?,?,?,?,?)');
                $q->execute([$v['nama'], $kat, $harga, $satuanSimpan, $hargaSatuan, $isiKg, $stok, $v['deskripsi'] ?: null, $gambar]);
...
<p class="muted">Cara beli mengikuti kategori: Fresh Good hanya kg, Dry Good kg atau satuan, Lainnya hanya satuan.</p>
...
<form class="pform" method="post" enctype="multipart/form-data" novalidate id="produkForm">
  ...
  <div class="pform-main">
    ...
    <p class="hint" id="hintKategori"></p>
    ...
    <div class="two grp-satuan">
      <div>
        ...
        <input id="satuan" name="satuan" type="text" maxlength="20" list="satuan-list" placeholder="karung, pak, dus…" value="<?= e((string) $v['satuan']) ?>"<?= attr_err($err, 'satuan') ?>>
        <datalist id="satuan-list"><option value="karung"><option value="pak"><option value="dus"><option value="jerigen"><option value="bal"></datalist>
    ...
    <div class="two grp-isikg">
      <div>
        <label for="isi_kg">Berat 1 satuan (kg)</label>
        <input id="isi_kg" name="isi_kg" type="number" min="1" step="1" inputmode="numeric" value="<?= e((string) $v['isi_kg']) ?>"<?= attr_err($err, 'isi_kg') ?>>
        <p class="hint">Misalnya 1 karung beras = 25 kg.</p>
        <?= field_err($err, 'isi_kg') ?>
      </div>
    </div>
...
(function () {
  var aturan = {
    'Fresh Good': { kg: true,  satuan: false, isikg: false, hint: 'Fresh Good dijual per kg saja.', stok: 'Stok (kg)' },
    'Dry Good':   { kg: true,  satuan: true,  isikg: true,  hint: 'Dry Good bisa dibeli per kg atau per satuan.', stok: 'Stok (kg)' },
    'Lainnya':    { kg: false, satuan: true,  isikg: false, hint: 'Lainnya dijual per satuan saja.', stok: 'Stok (jumlah satuan)' }
  };
  var sel = document.getElementById('kategori');
  function terapkan() {
    var a = aturan[sel.value] || { kg: true, satuan: true, isikg: true, hint: '', stok: 'Stok' };
    document.querySelectorAll('.grp-kg').forEach(function (el) { el.style.display = a.kg ? '' : 'none'; });
    document.querySelectorAll('.grp-satuan').forEach(function (el) { el.style.display = a.satuan ? '' : 'none'; });
    document.querySelectorAll('.grp-isikg').forEach(function (el) { el.style.display = a.isikg ? '' : 'none'; });
    document.getElementById('harga').required = a.kg;
    document.getElementById('satuan').required = a.satuan;
    document.getElementById('harga_satuan').required = a.satuan;
    document.getElementById('isi_kg').required = a.isikg;
    document.getElementById('hintKategori').textContent = a.hint;
    document.getElementById('lblStok').textContent = a.stok;
  }
